...
fn test_submit_path_with_absolute_path() {
    ...
    assert_eq!(
        state.settings.settings().backups_interval,
        Some(BackupsInterval::Daily)
    );
    assert_eq!(
        state.settings.settings().runtime_backups_interval,
...
fn test_submit_path_with_relative_path() {
    ...
    assert_eq!(
        state.settings.settings().backups_path,
        Some("/mock/home/backups".to_string())
    );
}
...
fn test_cycle_backup_interval() {
    ...
    settings.cycle_backup_interval();
    assert_eq!(
        settings.backups_interval,
        Some(BackupsInterval::Every2Weeks)
    );
...
fn test_cycle_runtime_backup_interval() {
    ...
    assert_eq!(
        settings.runtime_backups_interval,
        Some(RuntimeBackupsInterval::Every2Hours)
    );
    ...
    assert_eq!(
        settings.runtime_backups_interval,
        Some(RuntimeBackupsInterval::Every4Hours)
    );

    settings.cycle_runtime_backup_interval();
    assert_eq!(
        settings.runtime_backups_interval,
        Some(RuntimeBackupsInterval::Every6Hours)
    );

    settings.cycle_runtime_backup_interval();
    assert_eq!(
        settings.runtime_backups_interval,
        Some(RuntimeBackupsInterval::Every12Hours)
    );

    settings.cycle_runtime_backup_interval();
    assert_eq!(
        settings.runtime_backups_interval,
        Some(RuntimeBackupsInterval::Hourly)
    );
}
...
fn test_settings_serialization() {
    ...
    settings
        .backup_dates
        .insert("/test/map.json".to_string(), test_date);
    ...
    assert_eq!(
        deserialized.runtime_backups_interval,
        Some(RuntimeBackupsInterval::Every4Hours)
    );
...
fn test_settings_with_populated_backup_dates() {
    ...
    settings
        .backup_dates
        .insert("/path/map1.json".to_string(), date1);
    settings
        .backup_dates
        .insert("/path/map2.json".to_string(), date2);
...
fn test_settings_round_trip() {
    ...
    state.settings.settings_mut().runtime_backups_interval =
        Some(RuntimeBackupsInterval::Every4Hours);
