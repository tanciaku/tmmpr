...
use crate::{
    input::AppAction,
    states::start::{
        FocusedInputBox, RecentPaths, SelectedStartButton, StartState, get_recent_paths_with_fs,
    },
...
fn test_submit_path_create_new_file() {
    ...
    match result {
        AppAction::CreateMapFile(path) => {
            ...
            assert_eq!(
                path,
                PathBuf::from("/mock/home/test_maps_temp/new_map.json")
            );
        }
