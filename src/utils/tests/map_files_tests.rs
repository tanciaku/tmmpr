...
use crate::{
    ...
    utils::{
        IoErrorKind, MapData, create_map_file_with_fs, filesystem::test_utils::TempFileSystem,
        load_map_file_with_fs, read_json_data, save_map_file, save_with_notification,
...
fn create_populated_map_state(path: PathBuf) -> MapState {
    ...
    map_state
        .notes_state
        .add(10, 20, String::from("Test Note 1"), Color::White);
    map_state
        .notes_state
...
fn test_save_map_file_shows_save_success_notification() {
    ...
    assert_eq!(
        map_state.ui_state.show_notification,
        Some(Notification::SaveSuccess)
    );
...
fn test_save_map_file_preserves_note_properties() {
    ...
    map_state
        .notes_state
        .add(100, 200, String::from("Important Note"), Color::Cyan);
...
fn test_load_map_file_loads_note_properties() {
    ...
    map_state
        .notes_state
        .add(123, 456, String::from("Specific Note"), Color::Magenta);
...
fn test_load_map_file_loads_connections() {
    ...
    if let Screen::Map(loaded_state) = &app.screen {
        ...
        assert_eq!(
            loaded_state.connections_state.connections()[0].to_id,
            Some(1)
        );
        assert_eq!(
            loaded_state.connections_state.connections()[0].from_side,
            Side::Bottom
        );
        assert_eq!(
...
fn test_load_map_file_with_many_notes() {
    ...
    for i in 0..50 {
        map_state
            .notes_state
            .add(i * 10, i * 20, format!("Note {}", i), Color::White);
    }
    ...
    if let Screen::Map(loaded_state) = &app.screen {
        ...
        assert_eq!(
            loaded_state.notes_state.notes().get(&0).unwrap().content,
            "Note 0"
        );
        assert_eq!(
            loaded_state.notes_state.notes().get(&25).unwrap().content,
            "Note 25"
        );
        assert_eq!(
            loaded_state.notes_state.notes().get(&49).unwrap().content,
            "Note 49"
        );
    }
}
...
fn test_roundtrip_save_and_load_preserves_all_data() {
    ...
    original_state
        .notes_state
        .add(10, 20, String::from("First"), Color::Red);
    original_state
        .notes_state
        .add(30, 40, String::from("Second"), Color::Green);
    original_state
        .notes_state
    ...
    if let Screen::Map(loaded_state) = &app.screen {
        ...
        assert_eq!(
            loaded_state.notes_state.notes().get(&1).unwrap().content,
            "Second"
        );
        assert_eq!(
            loaded_state.notes_state.notes().get(&2).unwrap().content,
            "Third"
        );
        ...
        assert_eq!(
            loaded_state.connections_state.connections()[1].to_id,
            Some(1)
        );
...
fn test_roundtrip_create_save_load() {
    ...
    if let Screen::Map(map_state) = &mut app.screen {
        map_state
            .notes_state
            .add(100, 200, String::from("Created Note"), Color::Magenta);
...
fn test_multiple_save_load_cycles() {
    ...
    map_state
        .notes_state
        .add(10, 10, String::from("V1"), Color::White);
    let _ = save_map_file(&mut map_state, &file_path);
    ...
    if let Screen::Map(state) = &mut app.screen {
        state
            .notes_state
            .add(20, 20, String::from("V2"), Color::White);
    ...
    if let Screen::Map(state) = &mut app2.screen {
        state
            .notes_state
            .add(30, 30, String::from("V3"), Color::White);
        let _ = save_map_file(state, &file_path);
    }
