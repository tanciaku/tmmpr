...
fn test_backspace_at_beginning() {
    ...
    map_state.cursor_pos = 0; // Cursor at beginning
    
    backspace_char(&mut map_state, 0);
    
    // Should not change anything when cursor is at position 0
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Hello");
...
fn test_backspace_in_middle() {
    ...
    let note = Note::new(10, 10, String::from("Hello"), false, Color::White);
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 3; // Cursor after "Hel"
    
    backspace_char(&mut map_state, 0);
    
    // Should remove the 'l' before the cursor
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Helo");
    assert_eq!(map_state.cursor_pos, 2);
}
...
fn test_backspace_at_end() {
    ...
    let note = Note::new(10, 10, String::from("Hello"), false, Color::White);
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 5; // Cursor at end
    
    backspace_char(&mut map_state, 0);
    
    // Should remove the last character
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Hell");
    assert_eq!(map_state.cursor_pos, 4);
}
...
fn test_backspace_single_character() {
    ...
    assert_eq!(map_state.notes.get(&0).unwrap().content, "");
    assert_eq!(map_state.cursor_pos, 0);
}
...
fn test_backspace_with_newlines() {
    ...
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 6; // Cursor at start of "World"
    
    backspace_char(&mut map_state, 0);
    
    // Should remove the newline character
    assert_eq!(map_state.notes.get(&0).unwrap().content, "HelloWorld");
    assert_eq!(map_state.cursor_pos, 5);
}
...
fn test_backspace_unicode() {
    ...
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 7; // After "Hello 世"
    
    backspace_char(&mut map_state, 0);
    
    // Should remove the Chinese character '世'
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Hello 界");
    assert_eq!(map_state.cursor_pos, 6);
}

#[test]
fn test_backspace_nonexistent_note() {
    let mut map_state = create_test_map_state();
    map_state.cursor_pos = 5;
...
fn test_backspace_multiple_times() {
    ...
    assert_eq!(map_state.notes.get(&0).unwrap().content, "He");
    assert_eq!(map_state.cursor_pos, 2);
}
...
fn test_remove_in_middle() {
    ...
    let note = Note::new(10, 10, String::from("Hello"), false, Color::White);
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 2; // Cursor at 'l'
    
    remove_char(&mut map_state, 0);
    
    // Should remove the 'l' at the cursor
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Helo");
    assert_eq!(map_state.cursor_pos, 2); // Cursor stays in place
}
...
fn test_remove_at_beginning() {
    ...
    assert_eq!(map_state.notes.get(&0).unwrap().content, "ello");
    assert_eq!(map_state.cursor_pos, 0);
}
...
fn test_remove_with_newlines() {
    ...
    let note = Note::new(10, 10, String::from("Hello\nWorld"), false, Color::White);
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 5; // Cursor at newline
...
fn test_remove_cursor_adjustment() {
    ...
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 1; // At 'B'
    
    remove_char(&mut map_state, 0);
    
    // Should remove 'B' and adjust cursor
    assert_eq!(map_state.notes.get(&0).unwrap().content, "A");
    assert_eq!(map_state.cursor_pos, 0); // Cursor moved back
}
...
fn test_remove_multiple_times() {
    ...
    assert_eq!(map_state.notes.get(&0).unwrap().content, "lo");
    assert_eq!(map_state.cursor_pos, 0);
}
...
fn test_insert_at_beginning() {
    ...
    assert_eq!(map_state.notes.get(&0).unwrap().content, "XHello");
    assert_eq!(map_state.cursor_pos, 1);
...
fn test_insert_in_middle() {
    ...
    let note = Note::new(10, 10, String::from("Hello"), false, Color::White);
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 2; // After "He"
    
    insert_char(&mut map_state, 0, 'X');
    
    assert_eq!(map_state.notes.get(&0).unwrap().content, "HeXllo");
    assert_eq!(map_state.cursor_pos, 3);
}
...
fn test_insert_unicode() {
    ...
    let note = Note::new(10, 10, String::from("Hello "), false, Color::White);
    map_state.notes.insert(0, note);
    map_state.cursor_pos = 6;
    
    insert_char(&mut map_state, 0, '世');
    
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Hello 世");
    assert_eq!(map_state.cursor_pos, 7);
}
...
fn test_insert_special_characters() {
    ...
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Test !@#");
    assert_eq!(map_state.cursor_pos, 8);
}
...
fn test_move_up_on_first_line() {
    ...
    map_state.cursor_pos = 3; // On first line
...
fn test_move_up_to_same_column() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 8; // On 'r' in "World"
...
fn test_move_up_snap_to_shorter_line() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 10; // On 'r' in "Longer"
...
fn test_move_up_from_third_line() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 14; // On 'h' in "Third"
...
fn test_move_up_at_line_start() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 6; // At start of "World"
...
fn test_move_up_at_line_end() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 11; // At end of "World"
...
fn test_move_up_no_selected_note() {
    ...
    let note = Note::new(10, 10, String::from("Hello\nWorld"), false, Color::White);
    map_state.notes.insert(0, note);
    map_state.selected_note = None; // No selected note
    map_state.cursor_pos = 8;
...
fn test_move_up_empty_lines() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 7; // At start of "World"
...
fn test_move_up_varying_line_lengths() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 30; // On 'd' in "Med"
...
fn test_move_down_on_last_line() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 8; // On second line
...
fn test_move_down_to_same_column() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 2; // On 'l' in "Hello"
...
fn test_move_down_snap_to_shorter_line() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 7; // On 'l' in "line"
    
    move_cursor_down(&mut map_state);
    
    // Should snap to end of shorter next line (position 14 - end of "Hi")
    assert_eq!(map_state.cursor_pos, 14);
}
...
fn test_move_down_from_first_line() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 2; // On 'r' in "First"
...
fn test_move_down_at_line_start() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 0; // At start of "Hello"
...
fn test_move_down_at_line_end() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 5; // At end of "Hello"
    
    move_cursor_down(&mut map_state);
    
    // Should move to end of next line
    assert_eq!(map_state.cursor_pos, 11);
}
...
fn test_move_down_empty_lines() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 3; // On 'l' in "Hello"
...
fn test_move_down_varying_line_lengths() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 2; // On 'd' in "Med"
...
fn test_move_down_to_last_line() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 8; // On 'c' in "Second"
    
    move_cursor_down(&mut map_state);
    
    // Should move to third line (position 15 - 'i' in "Third")
    assert_eq!(map_state.cursor_pos, 15);
}
...
fn test_edit_workflow_insert_and_backspace() {
    ...
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Hel");
    assert_eq!(map_state.cursor_pos, 3);
}
...
fn test_edit_workflow_navigation_and_editing() {
    ...
    // Move down to line 2
    move_cursor_down(&mut map_state);
    assert_eq!(map_state.cursor_pos, 7); // Start of "Line 2"
    
    // Insert an 'X'
    insert_char(&mut map_state, 0, 'X');
    assert_eq!(map_state.notes.get(&0).unwrap().content, "Line 1\nXLine 2\nLine 3");
    assert_eq!(map_state.cursor_pos, 8); // Cursor moved after insert
...
fn test_cursor_movement_up_and_down() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 6; // On third 'B' in middle line (column 2)
    
    // Move down (should snap to end of shorter line)
    move_cursor_down(&mut map_state);
    assert_eq!(map_state.cursor_pos, 12); // End of "CC"
    
    // Move up (column 2 in "BBBBB")
    move_cursor_up(&mut map_state);
    assert_eq!(map_state.cursor_pos, 6); // Back to third 'B' in "BBBBB"
    
    // Move up again (column 2 in "AAA")
    move_cursor_up(&mut map_state);
    assert_eq!(map_state.cursor_pos, 2); // Third 'A' in "AAA"
}
...
fn test_multiline_editing() {
    ...
    move_cursor_up(&mut map_state);
    assert_eq!(map_state.cursor_pos, 5); // End of "Hello"
