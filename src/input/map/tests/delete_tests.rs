...
fn test_delete_kh_escape_switches_to_visual() {
    ...
    assert_eq!(map_state.selected_note, Some(0)); // Should remain selected
}
...
fn test_delete_single_note() {
    ...
    assert!(map_state.notes.is_empty()); // Note should be removed
    assert!(map_state.render_order.is_empty()); // Render order should be empty
    assert_eq!(map_state.selected_note, None); // No selected note
...
fn test_delete_note_with_multiple_notes() {
    ...
    map_state.notes.insert(2, Note::new(80, 40, String::from("Note 2"), false, Color::White));
    
    map_state.render_order = vec![0, 1, 2];
    map_state.selected_note = Some(1); // Select middle note
    map_state.current_mode = Mode::Delete;
    ...
    assert_eq!(map_state.notes.len(), 2);
    assert!(map_state.notes.contains_key(&0));
    assert!(!map_state.notes.contains_key(&1)); // This one should be deleted
    assert!(map_state.notes.contains_key(&2));
    
    // Check render order
    assert_eq!(map_state.render_order, vec![0, 2]);
...
fn test_delete_note_with_multiple_connections() {
    ...
    map_state.render_order = vec![0, 1, 2];
    map_state.selected_note = Some(1); // Select middle note to delete
    ...
    map_state.connections.extend_from_slice(&[connection1, connection2, connection3]);
    
    // Set up connection index
    map_state.connection_index.entry(0).or_default().extend_from_slice(&[connection1, connection3]);
    map_state.connection_index.entry(1).or_default().extend_from_slice(&[connection1, connection2]);
    map_state.connection_index.entry(2).or_default().extend_from_slice(&[connection2, connection3]);
...
fn test_delete_note_as_connection_target() {
    ...
    map_state.selected_note = Some(0); // Select note that is the target
    ...
    // Connection index should be cleaned up
    assert!(!map_state.connection_index.contains_key(&0));
    if let Some(connections) = map_state.connection_index.get(&1) {
...
fn test_delete_note_as_connection_source() {
    ...
    map_state.render_order = vec![0, 1];
    map_state.selected_note = Some(0); // Select note that is the source
...
fn test_delete_kh_other_keys_ignored() {
    ...
    for key in test_keys {
        ...
        assert_eq!(map_state.notes.len(), 1); // Note should still be there
        assert_eq!(map_state.can_exit, true); // Should remain unchanged
    }
}
...
fn test_delete_note_render_order_edge_cases() {
    ...
    map_state.notes.insert(0, Note::new(10, 10, String::from("Note 0"), false, Color::White));
    map_state.notes.insert(1, Note::new(50, 25, String::from("Note 1"), false, Color::White));
    map_state.notes.insert(2, Note::new(80, 40, String::from("Note 2"), true, Color::White));
    
    // Test deleting first note in render order
    map_state.render_order = vec![2, 1, 0]; // Note 2 is first
    map_state.selected_note = Some(2);
    map_state.current_mode = Mode::Delete;

    let result = map_delete_kh(&mut map_state, create_key_event(KeyCode::Char('d')));

    assert_eq!(result, AppAction::Continue);
    assert_eq!(map_state.render_order, vec![1, 0]); // Note 2 removed from front
    assert_eq!(map_state.notes.len(), 2);
    assert!(!map_state.notes.contains_key(&2));
}
...
fn test_delete_note_last_in_render_order() {
    ...
    map_state.render_order = vec![0, 1, 2]; // Note 2 is last
    map_state.selected_note = Some(2);
    map_state.current_mode = Mode::Delete;
    ...
    assert_eq!(result, AppAction::Continue);
    assert_eq!(map_state.render_order, vec![0, 1]); // Note 2 removed from end
    assert_eq!(map_state.notes.len(), 2);
    assert!(!map_state.notes.contains_key(&2));
}
