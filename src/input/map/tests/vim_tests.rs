...
fn test_jump_forward_a_word_already_at_end() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 10; // At 'd', the last character

    jump_forward_a_word(&mut map_state);

    // Should stay at the end since there are no more words
    assert_eq!(map_state.cursor_pos, 10);
}
...
fn test_jump_forward_a_word_multiple_jumps() {
    ...
    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 4); // 'q' in "quick"

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 10); // 'b' in "brown"

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 16); // 'f' in "fox"

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 18); // Last character 'x'
}
...
fn test_jump_forward_a_word_from_middle_of_word() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 2; // At 'l' in "hello"
...
fn test_jump_forward_a_word_single_character_word() {
    ...
    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 2); // 'b'

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 4); // 'c'
}
...
fn test_jump_forward_a_word_multiline_text() {
    ...
    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 6); // 'l' in "line"

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 11); // 's' in "second"
}
...
fn test_jump_back_a_word_simple_two_words() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 6; // At 'w' in "world"
...
fn test_jump_back_a_word_from_middle_of_word() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 8; // At 'r' in "world"
...
fn test_jump_back_a_word_from_end_of_word() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 10; // At 'd' in "world"
...
fn test_jump_back_a_word_multiple_spaces() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 8; // At 'w' in "world"
...
fn test_jump_back_a_word_multiple_jumps() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 18; // At 'x' in "fox"
    ...
    jump_back_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 0); // 't' in "the"
}
...
fn test_jump_back_a_word_from_space_between_words() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 5; // At space between "hello" and "world"
...
fn test_jump_back_a_word_single_character_words() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 4; // At 'c'

    jump_back_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 2); // 'b'

    jump_back_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 0); // 'a'
}
...
fn test_jump_back_a_word_multiline_text() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 27; // At 'd' in "third"

    jump_back_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 23); // 't' in "third"

    jump_back_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 18); // 'l' in "line" (second)
...
fn test_jump_back_a_word_mixed_whitespace() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 8; // At 'w' in "world" (position 8)
...
fn test_jump_forward_then_back() {
    ...
    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 6); // 'w' in "world"

    jump_back_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 0); // Back to 'h' in "hello"
}
...
fn test_jump_back_then_forward() {
    ...
    map_state.notes.insert(0, note);
    map_state.selected_note = Some(0);
    map_state.cursor_pos = 12; // 't' in "test"

    jump_back_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 6); // 'w' in "world"

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 12); // Back to 't' in "test"
}
...
fn test_navigation_through_complex_text() {
    ...
    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 5); // 'i' in "is"

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 8); // 'a'

    jump_forward_a_word(&mut map_state);
    assert_eq!(map_state.cursor_pos, 10); // 't' in "test"
