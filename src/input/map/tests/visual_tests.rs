...
use crate::{
    input::{map::visual::map_visual_kh, AppAction},
...
fn test_visual_escape_to_normal_mode() {
    ...
    // Note should be deselected
    assert_eq!(map_state.notes.get(&0).unwrap().selected, false);
}
...
fn test_visual_enter_move_mode() {
    ...
    assert_eq!(result, AppAction::Continue);
    assert_eq!(map_state.visual_move, true);
...
fn test_visual_cycle_note_color() {
    ...
    let original_color = map_state.notes.get(&0).unwrap().color;
    ...
    let new_color = map_state.notes.get(&0).unwrap().color;
    assert_ne!(original_color, new_color); // Color should have changed
}

#[test]
fn test_visual_cycle_note_color_no_selected_note() {
    // Shouldn't happen
...
fn test_visual_create_new_connection() {
    ...
    assert_eq!(result, AppAction::Continue);
    assert_eq!(map_state.visual_connection, true);
    assert_eq!(map_state.can_exit, false); // Should mark as dirty
    
    // Should have created a focused connection
    assert!(map_state.focused_connection.is_some());
    let connection = map_state.focused_connection.unwrap();
...
fn test_visual_enter_connection_mode_with_existing_connection() {
    ...
    // Add two notes
    map_state.notes.insert(0, Note::new(10, 10, String::from("Note 0"), true, Color::White));
    map_state.notes.insert(1, Note::new(50, 25, String::from("Note 1"), false, Color::White));
    map_state.selected_note = Some(0);
    map_state.current_mode = Mode::Visual;
    ...
    assert_eq!(result, AppAction::Continue);
    assert_eq!(map_state.visual_connection, true);
    assert_eq!(map_state.visual_editing_a_connection, true);
    
    // Connection should be in focus
    assert!(map_state.focused_connection.is_some());
    assert_eq!(map_state.editing_connection_index, Some(0));
}
...
fn test_visual_switch_focus_down() {
    ...
    map_state.notes.insert(0, Note::new(50, 10, String::from("Note 0"), true, Color::White));
    map_state.notes.insert(1, Note::new(50, 30, String::from("Note 1"), false, Color::White));
    map_state.selected_note = Some(0);
    map_state.current_mode = Mode::Visual;
...
fn test_visual_switch_focus_with_arrow_keys() {
    ...
    map_state.notes.insert(0, Note::new(50, 25, String::from("Note 0"), true, Color::White));
    map_state.notes.insert(1, Note::new(70, 25, String::from("Note 1"), false, Color::White));
    map_state.selected_note = Some(0);
    map_state.current_mode = Mode::Visual;
...
fn test_move_mode_unhandled_keys() {
    ...
    for key in test_keys {
        ...
        assert_eq!(result, AppAction::Continue);
        assert_eq!(map_state.visual_move, true); // Should remain in move mode
    }
}
...
fn test_connection_mode_rotate_to_side() {
    ...
    map_state.notes.insert(0, Note::new(10, 10, String::from("Note 0"), false, Color::White));
    map_state.notes.insert(1, Note::new(50, 25, String::from("Note 1"), true, Color::White));
    map_state.selected_note = Some(1); // Select the target note
    map_state.current_mode = Mode::Visual;
    map_state.visual_connection = true;
...
#[test]
fn test_connection_mode_rotate_no_selected_note() {
    // Shouldn't happen
    ...
    assert_eq!(result, AppAction::Continue);
    // Should not change anything without selected note
    let connection = map_state.focused_connection.unwrap();
    assert_eq!(connection.from_side, Side::Top); // Should not have changed
}
...
fn test_connection_mode_cycle_connections() {
    ...
    map_state.notes.insert(0, Note::new(10, 10, String::from("Note 0"), true, Color::White));
    map_state.notes.insert(1, Note::new(50, 25, String::from("Note 1"), false, Color::White));
    map_state.notes.insert(2, Note::new(90, 40, String::from("Note 2"), false, Color::White));
    map_state.selected_note = Some(0);
    map_state.current_mode = Mode::Visual;
    map_state.visual_connection = true;
    ...
    map_state.connections.push(connection2);
    map_state.connections.push(connection3);
    
    // Set up focused connection and editing index (connection1 was at index 0)
    map_state.focused_connection = Some(connection1);
    map_state.editing_connection_index = Some(0);

    // First cycle - should move to connection2
    let result = map_visual_kh(&mut map_state, create_key_event(KeyCode::Char('n')));
    assert_eq!(result, AppAction::Continue);
    assert!(map_state.focused_connection.is_some());
    let focused = map_state.focused_connection.unwrap();
...
fn test_connection_mode_cycle_connections_not_editing() {
    ...
    map_state.notes.insert(0, Note::new(10, 10, String::from("Note 0"), true, Color::White));
    map_state.notes.insert(1, Note::new(50, 10, String::from("Note 1"), false, Color::White));
    map_state.selected_note = Some(0);
    map_state.current_mode = Mode::Visual;
    map_state.visual_connection = true;
    map_state.visual_editing_a_connection = false; // Creating a new connection, not editing existing
    ...
    map_state.focused_connection = Some(focused_connection);
    ...
    map_state.connections.push(existing_connection);
    ...
    assert_eq!(map_state.connections[0].color, Color::Green);
}
...
fn test_connection_mode_delete_connection() {
    ...
    assert_eq!(result, AppAction::Continue);
    assert_eq!(map_state.can_exit, false); // Should mark as dirty
    assert_eq!(map_state.focused_connection, None); // Connection should be deleted
    assert_eq!(map_state.visual_connection, false); // Should exit connection mode
    assert_eq!(map_state.visual_editing_a_connection, false);
    assert_eq!(map_state.editing_connection_index, None);
}
...
fn test_connection_mode_delete_not_editing() {
    ...
    map_state.current_mode = Mode::Visual;
    map_state.visual_connection = true;
    map_state.visual_editing_a_connection = false; // Not editing
...
fn test_connection_mode_unhandled_keys() {
    ...
    for key in test_keys {
        let result = map_visual_kh(&mut map_state, create_key_event(key));
        assert_eq!(result, AppAction::Continue);
        assert_eq!(map_state.visual_connection, true); // Should remain in connection mode
    }
}
...
fn test_visual_mode_no_selected_note() {
    ...
    assert_eq!(result, AppAction::Continue);
    assert_eq!(map_state.visual_move, true); // Should still enter move mode
}
...
fn test_multiple_notes_with_connections() {
    ...
    map_state.notes.insert(0, Note::new(10, 10, String::from("Note 0"), true, Color::White));
    map_state.notes.insert(1, Note::new(50, 10, String::from("Note 1"), false, Color::Green));
    map_state.notes.insert(2, Note::new(90, 10, String::from("Note 2"), false, Color::Blue));
    map_state.selected_note = Some(0);
    map_state.current_mode = Mode::Visual;
    ...
    map_state.connections.extend_from_slice(&[connection1, connection2]);
    ...
    let focused_connection = map_state.focused_connection.unwrap();
...
#[test]
fn test_create_connection_no_selected_note() {
    // Shouldn't happen
    ...
    assert!(map_state.focused_connection.is_none());
    assert_eq!(map_state.visual_connection, false);
}
...
fn test_all_direction_keys_consistency() {
    ...
    map_state.notes.insert(0, Note::new(50, 25, String::from("Test Note"), true, Color::White));
    map_state.selected_note = Some(0);
    map_state.current_mode = Mode::Visual;
...
fn test_stash_and_cycle_connections_wraparound() {
    ...
    // Cycle - should wrap around to find the same connection again
    // When cycles: first stashes the focused connection into vec,
    // after which it performs a search - since this is the only
    // connection associated with this note - it finds it and places
    // it back into focused_connection.
